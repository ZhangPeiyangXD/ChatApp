...
本文档详细介绍了如何通过自定义注解[@GlobalInterceptor](file:///D:/chat_learn/easychat-java/src/main/java/com/itzpy/annotation/GlobalInterceptor.java)和AOP切面[GlobalOperationAspect](file:///D:/chat_learn/easychat-java/src\..\..\src\main\java\com\itzpy\aspect\GlobalOperationAspect.java)实现系统的登录校验功能。这是一种优雅的方式，可以在不侵入业务代码的前提下，对需要登录验证的接口进行统一的身份认证。
...
GlobalInterceptor注解用于标记需要进行登录校验的控制器方法。(GlobalInterceptor.java)
...
GlobalOperationAspect是实现登录校验的核心切面类。(GlobalOperationAspect.java)
...
在控制器方法上使用@GlobalInterceptor注解：(GlobalInterceptor.java)
...
1. **注解定义**：通过@Target(ElementType.METHOD)和@Retention(RetentionPolicy.RUNTIME)定义运行时可见的方法级注解(GlobalInterceptor.java第9-10行)

2. **切点表达式**：`@Pointcut("@annotation(com.itzpy.annotation.GlobalInterceptor)")` 匹配所有被@GlobalInterceptor注解标记的方法(GlobalInterceptor.java第11-14行)

3. **前置通知**：@Before("pt()")在目标方法执行前进行拦截处理(GlobalOperationAspect.java第49-59行)
...
通过自定义注解@GlobalInterceptor和AOP切面GlobalOperationAspect，我们实现了以下优势：(GlobalInterceptor.java和GlobalOperationAspect.java)
